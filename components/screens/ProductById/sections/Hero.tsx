...
const HeroSection = ({ product }: IProductByIdPageProps) => {
	...
	const renderFeature = () => {
		switch (product.handle) {
			...
			return <div className={classes.productPageFeatures} >
					<div className={classes.overLayFeatures} ></div>
					<div  >
					{product.filesIncluded.details.length   ? <ul className={classes.productPageDetailsUl}>
						{product.filesIncluded.details.map((el , index) =>  (<li key={index} >{el}</li>))}
					</ul> : ''}
					</div>
			</div>
			...
			<div>
			{product.features.length ? <ul>
				{product.features.map((el:any, index:any) => <li key={index}>{el}</li> ) }
			</ul> :  ''}
			{product.filesIncluded.details.length   ? <ul>
				{ product.filesIncluded.count ? <li className={classes.productPageIncludeLi} >include: {product.filesIncluded.count} files</li> : ''}
				{product.filesIncluded.details.map((el , index) =>  (<li key={index} >{el}</li>))}
			</ul> : ''}
			</div>
		</div>
	...
	return (
		<section className={classes.productPageSection}>
			<div className={classes.productPageContainer}>
				...
				{renderFeature()}
				{product.video ? <div className={classes.productPageYoutubeSections} >
						<h4>
							{product.video.title}
						</h4>
						<div>
							<div className={classes.overLayYoutubeSection} ></div>
							 ...
							 {product.video.srcTwo ?<iframe src={product.video.srcTwo} />:""}
						</div>
				</div> : ''}
			</div>
		</section>
