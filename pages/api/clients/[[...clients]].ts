...
const clientsAPIRouteHandler = nextConnect((router) =>
	router
		...
		.post(
			`${basePath}/subscribe-newsletters`,
			clientsController.subscribeToNewsLetters,
		)
		.post(`${basePath}/contact-us`, clientsController.supportForm)
		.post(`${basePath}/redeem-code`, clientsController.redeemCode)
		.post(
			`${basePath}/create-redeem-order`,
			clientsController.createOrderRedeemCode,
		)
